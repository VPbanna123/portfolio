...
import { experiencesData, hackathonsData } from '@/lib/data';
  ...
  return (
    ...
    >
      <SectionHeading
        ...
        content="Professional experience and internships."
      />
      ...
      <div className="mt-16">
        <SectionHeading
          ...
          content="Competition wins and hackathon achievements."
        />
        <div className="relative max-w-screen-md">
          {hackathonsData.map(
            ({ title, description, event, year, technologies }) => (
              <div
                key={title}
                className="not-last:pb-12 relative pl-8 [&:not(:last-child)]:pb-10"
              >
                <div className="bg-muted absolute left-0 top-2.5 h-full w-[2px] group-first:top-6 group-first:h-[calc(100%-24px)]">
                  <div className="border-yellow-500 bg-background absolute left-[-5px] top-0 size-3 rounded-full border-2" />
                </div>
                <motion.div
                  initial={{ opacity: 0, y: 100 }}
                  whileInView={{
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="flex size-9 shrink-0 items-center justify-center rounded-full border border-yellow-500/50 bg-yellow-500/10">
                      <Icons.star className="size-5 text-yellow-500" />
                    </div>
